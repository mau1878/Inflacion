...
# Agregar una nueva casilla de verificación para el cálculo alternativo de porcentaje
...
if tickers_input:
      ...
      try:
          ...
          if show_percentage or show_percentage_from_recent:
              if show_percentage_from_recent:
                  # Calcular la variación porcentual desde el valor más reciente
                  stock_data['Inflation_Adjusted_Percentage'] = (
                      (stock_data['Inflation_Adjusted_Close'].iloc[-1] / stock_data['Inflation_Adjusted_Close']) - 1
                  ) * 100
                  # Limitar el valor mínimo al -100%
                  stock_data['Inflation_Adjusted_Percentage'] = stock_data['Inflation_Adjusted_Percentage'].clip(lower=-100)
              else:
                  # Calcular porcentajes usando el valor inicial como referencia
                  stock_data['Inflation_Adjusted_Percentage'] = (
                      stock_data['Inflation_Adjusted_Close'] / stock_data['Inflation_Adjusted_Close'].iloc[0] - 1
                  ) * 100

              # Graficar las variaciones porcentuales
              fig.add_trace(
                  go.Scatter(
                      x=stock_data.index,
              ...
              # Graficar el precio promedio como línea punteada
  ...
  if custom_expression:
      try:
          ...
          if show_percentage or show_percentage_from_recent:
              if show_percentage_from_recent:
                  # Calcular porcentajes usando el valor más reciente como referencia
                  custom_series_pct = (adjusted_series / adjusted_series.iloc[-1] - 1) * 100
                  # Invertir el signo para valores pasados
